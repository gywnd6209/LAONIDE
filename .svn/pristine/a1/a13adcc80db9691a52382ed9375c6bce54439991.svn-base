...
<%-- 									</c:if> --%>
			...
			<div>
				<form name="searchFrm" action="${pageContext.request.contextPath }/admin/freeboard/freeboardList.do" method="post"
...
// 		var bo_title = $(this).find('td:eq(1)').text();
		...
		$(location).attr("href","${pageContext.request.contextPath}/admin/freeboard/freeboardView.do?bo_no="+ bo_no +"&currentPage=${currentPage}&search_keyword=" + encodeURI('${search_keyword}')+"&search_keycode=${search_keycode}");
