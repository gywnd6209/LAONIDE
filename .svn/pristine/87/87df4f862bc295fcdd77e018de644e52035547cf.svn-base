    ...
    return (function e(t, n, r) {
        ...
        2: [function(require, module, exports) {
            ...
            var CompositionHelper = (function() {
                ...
                CompositionHelper.prototype.compositionstart = function() {
                    ...
                    if (this.terminal.browser.isMSIE) {
                        this.compositionPosition.start = this.textarea.value.length - 1;
                    } else {
                    ...
                    if (char >= ' ') {
                        ...
                        if (this._terminal.x + ch_width - 1 >= this._terminal.cols) {
                            if (this._terminal.wraparoundMode) {
                                this._terminal.x = 0;
                                // goorm start
                                this._terminal.lines.get(this._terminal.y + this._terminal.ybase)['wordbreak'] = true;
                                // goorm end
                                this._terminal.y++;
                        ...
                        if (this._terminal.insertMode) {
                            for (var moves = 0; moves < ch_width; ++moves) {
                                ...
                                var currentWordbreak = this._terminal.lines.get(row)['wordbreak'];
                                this._terminal.lines.get(row).splice(this._terminal.x, 0, [this._terminal.curAttr, ' ', 1]);
                                this._terminal.lines.get(row)['wordbreak'] = currentWordbreak;
                                // goorm end
                            }
                        }
                                ...
                                if (ch === EscapeSequences_1.C0.ESC || ch === EscapeSequences_1.C0.BEL) {
                                    ...
                                    switch (this._terminal.prefix) {
                                        case '':
                                            break;
                                        case '$q':
                                            var pt = this._terminal.currentParam,
                                                valid = false;
                                            switch (pt) {
                                                case '"q':
                                                    pt = '0"q';
            ...
            var Renderer = (function() {
                ...
                Renderer.prototype._refresh = function(start, end) {
                    ...
                    for (; y <= end; y++) {
                        ...
                        // goorm start - minung.han
                        // for terminal path click
                        var result = '';
                        var div = this._terminal.children[y]; // get the div wrapper
                        for (var node_index = 0; node_index < div.childNodes.length; node_index++) { // loop over the nodes
                            var node = div.childNodes[node_index];
                ...
                Viewport.prototype.refresh = function() {
                    if (this.charMeasure.height > 0) {
                        ...
                        if (rowHeightChanged) {
                            ...
                            /* goorm start // Fix terminal line-height bug
        ...
        12: [function(require, module, exports) {
            ...
            var CharMeasure = (function(_super) {
                ...
                CharMeasure.prototype.measure = function(force) { // add force for resizing forcingly
                    // goorm end
                    var _this = this;
                ...
                CharMeasure.prototype._doMeasure = function(force) { // add force for resizing forcingly
                    // goorm end
        ...
        15: [function(require, module, exports) {
            ...
            Terminal.bindKeys = function(term) {
                ...
                // goorm start - minung.han
                // 이 기능은 termianl.context 기능에 포함되어 codemirror 기능은 제거
            ...
            Terminal.prototype.open = function(parent, focus) {
                ...
                this.context = this.parent.target[0].ownerDocument.defaultView;
                this.document = this.parent.target[0].ownerDocument;
                // goorm end
                ...
                this.parent.target[0].appendChild(this.element);
                // goorm end
                ...
                on(this.element, 'click', function(e) {
                    ...
                    // goorm start
                    if (selection.type === 'Range') { // goorm: Leave user selected text state
                        e.stopPropagation();
                    }
                    // goorm end
                    if (!isRange) {
            ...
            Terminal.prototype.evaluateKeyEscapeSequence = function(ev) {
                ...
                switch (ev.keyCode) {
                    ...
                    case 9:
                        ...
                        break;
                    case 13:
                        // goorm start // seho.ahn // Don't escape if alt+shift+enter(layout_switch) is pressed
                ...
                // goorm start
                if (ev.keyCode === 8221 || ev.keyCode === 8217) {
                    return {
                        shortcut: true
                    };
                } // goorm shortcut keypress keycode
                // goorm end
                return result;
            };
            ...
            // goorm start
            Terminal.prototype.getLength = function(line) {
                var i = line.length - 1;
                while (i >= 0) {
                    if (line[i][1] != ' ') {
                        break;
            ...
            Terminal.prototype.setBlank = function(line, x) {
                if (line.length > x) {
                    var currentWordbreak = line['wordbreak'];
                    line = line.slice(0, x);
                    line['wordbreak'] = currentWordbreak;
                } else if (line.length < x) {
                    var i = line.length;
                    while (i++ < x) {
                        line.push([this.defAttr, ' ', 1]);
                    }
                }
            };
            // Terminal.prototype.resize = function (x, y) {
            Terminal.prototype.resize = function(x, y, force) { // add force for resizing forcingly
                // goorm end
                if (isNaN(x) || isNaN(y)) {
                    return;
                }
                ...
                if (!force && x === this.cols && y === this.rows) { // if force is true, just go to resize
                    // goorm end
                    return;
                }
                if (x < 1)
                ...
                this.charMeasure.measure(force); // add force for resizing forcingly

                var last_line = this.lines.length;
                var last_text_line = 0;

                if (j < x) {
                    for (i = 0; i < last_line; i++) {
                        var currentLength = this.getLength(this.lines.get(i));

                        if (currentLength) {
                            last_text_line = i;
                        }

                        if (this.lines.get(i)['wordbreak'] === true) {
                            var currentWordbreak = this.lines.get(i + 1)['wordbreak'];
                            var str = this.lines.get(i + 1).splice(0, x - currentLength);

                            this.lines.get(i + 1)['wordbreak'] = currentWordbreak;
                            currentWordbreak = this.lines.get(i)['wordbreak'];
                            this.lines.set(i, this.lines.get(i).concat(str));
                            this.lines.get(i)['wordbreak'] = currentWordbreak;

                            if (this.getLength(this.lines.get(i + 1)) <= 0) {
                                this.lines.get(i)['wordbreak'] = this.lines.get(i + 1)['wordbreak'];
                                ...
                                i--; //seongho.cha: check this line with next next line.
                                last_line--;
                    ...
                    for (i = 0; i < last_line; i++) {
                        var curLineLength = this.getLength(this.lines.get(i));
                        ...
                        if (curLineLength > x) {
                            if (this.lines.get(i)['wordbreak'] !== true) {
                                this.lines.get(i)['wordbreak'] = true;
                            ...
                            var currentWordbreak = this.lines.get(i)['wordbreak'];
                            var str = this.lines.get(i).splice(x);

                            this.lines.get(i)['wordbreak'] = currentWordbreak;
                            currentWordbreak = this.lines.get(i + 1)['wordbreak'];
                            this.lines.set(i + 1, str.concat(this.lines.get(i + 1)));

                            var nextLineLength = this.getLength(this.lines.get(i + 1));
                            var index = (nextLineLength > x) ? nextLineLength : x;

                            this.lines.set(i + 1, this.lines.get(i + 1).slice(0, index));
                            this.lines.get(i + 1)['wordbreak'] = currentWordbreak;
                        } else if (this.lines.get(i).length > x) {
                            var currentWordbreak = this.lines.get(i)['wordbreak'];

                            this.lines.set(i, this.lines.get(i).slice(0, x));
                            this.lines.get(i)['wordbreak'] = currentWordbreak;
                        }
                    }
                }
                ...
                for (i = 0; i < this.lines.length; i++) {
                    this.setBlank(this.lines.get(i), x);
                }

                this.setupStops(j);
                this.cols = x;
                // resize rows
                if (last_text_line === 0) { // seho.ahn // 세로 리사이즈 시 last_text_line 정의
                    for (i = 0; i < last_line; i++) {
                        var currentLength = this.getLength(this.lines.get(i));
                ...
                // }
                // else {	// hidden by goorm
                // goorm start
                if (this.y > (y - 1)) {
                    this.ybase += (this.y - (y - 1));
                }
                j = this.rows;
                // goorm end
                while (j-- > y) {
                ...
                //     this.x = x - 1;
                // }
                /* seho.ahn // 리사이즈 후 커서 재배치 */
                this.x = this.getLength(this.lines.get(last_text_line)) + 1;
                this.y = last_text_line - this.ybase;
                if (this.y < 0) {
                    this.y = 0;
                }
                if (this.x === x) {
            ...
            Terminal.prototype.handler = function(data) {
                ...
                this.emit('data', data);
                // goorm start
                this.parent.socket.emit("pty_execute_command", JSON.stringify({
            ...
            "./Renderer": 8,
            "./Viewport": 9,
            "./handlers/Clipboard": 10,
            "./utils/Browser": 11,
            "./utils/CharMeasure": 12,
            "./utils/CircularList": 13
        }]
    }, {}, [15])(15)
});
