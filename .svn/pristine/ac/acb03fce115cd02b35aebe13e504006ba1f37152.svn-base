...
<!--						<input type="text" disabled="disabled" class="form-control"  id="bo_date" name="bo_date" /> -->
				...
				<div class="form-group row">
					...
					<div class="col-sm-10">
						...
						<c:if test="${LOGIN_MEMBERINFO.mem_id ne freeboardInfo.mem_id}">
...
<!-- 					<div class="col-sm-10"> -->
							...
							<i class="ti-save">&nbsp;${fileItem.file_name}
								<a href="${pageContext.request.contextPath}/user/freeboard/fileDownload.do?file_seq=${fileItem.file_seq }">
						...
						<div class="col-sm-12" style="padding-left: 90px; padding-right:90px; padding-top: 2px;  background-color: #F0F0F0;">
							<div class="row" >
								<div class="label-main" style=" float: left;">
									<c:if test="${LOGIN_MEMBERINFO.mem_nick eq freeboardReplyInfo.bo_writer}">
									<label class="label " style="background-color: #A27AAF;">  <font size="2pt" >${freeboardReplyInfo.bo_writer}</font></label>
									</c:if>
									<c:if test="${LOGIN_MEMBERINFO.mem_nick ne freeboardReplyInfo.bo_writer}">
...
function fn_updateReply(bo_no){
	...
	// 댓글 수정 ajax
	 $.ajax({
		type : 'POST',
		url : '${pageContext.request.contextPath}/user/freeboard/updateFreeboardReply.do',
		dataType : 'json', 
		data : { 
			 bo_cont 	: 	editContent,
...
function deleteReply(bo_no) {
	...
	$.ajax({
		type : 'POST',
		url : '${pageContext.request.contextPath}/user/freeboard/deleteFreeboardReply.do',
		dataType : 'json', 
		data : {
			bo_no : bo_no
		},
...
function fn_regi(){
	...
	$.ajax({
		type : 'POST',
		url : '${pageContext.request.contextPath}/user/freeboard/insertFreeboardReply.do',
		dataType : 'json', 
		data : { 
