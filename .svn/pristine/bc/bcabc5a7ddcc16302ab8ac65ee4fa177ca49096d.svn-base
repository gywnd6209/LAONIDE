...
<head>
<meta charset="UTF-8">
<title>이슈관리 게시판 목록</title>
<script type="text/javascript">
$(function () {
	$('#issueboardRegiBtn').click(function() {
		if(eval('${!empty LOGIN_MEMBERINFO}')){
			$(location).attr('href','${pageContext.request.contextPath }/user/issuetracker/issueboard/issueboardForm.do'
	...
	$('#issueboardTBL tr').click(function(){
		var isu_no 	= $(this).find('#isu_no').val();
		var rnum 	= $(this).find('#rnum').val();
		$(location).attr('href','${pageContext.request.contextPath}/user/issuetracker/issueboard/issueboardView.do?isu_no='+isu_no+'&rnum='+rnum
				+'&currentPage=${currentPage}&pro_code=${pro_code}&search_keyword=' + encodeURI('${search_keyword}')+'&search_keycode=${search_keycode}&pro_name=${param.pro_name }');
	});
             ...
             <div class="card-block table-border-style">
                 <div class="table-responsive">
                     <table class="table table-hover">
                         ...
                         <tbody id="issueboardTBL">
                         	 <c:if test="${!empty issueboardList }">
		                         <c:forEach items="${issueboardList }" var="issueboardList">
						            <tr style="text-align: center;">
						               <td><input type="hidden" id="isu_no" value="${issueboardList.isu_no }">
						               		<input type="hidden" id="rnum" value="${issueboardList.rnum }">
						               		${issueboardList.rnum }</td>
							              <c:if test="${issueboardList.isu_rank eq '낮음'}">
							               <td><img src="${pageContext.request.contextPath }/assets/images/priority/Urgency_lowness.png" style="height: 20px"></td>
							              </c:if>
							              <c:if test="${issueboardList.isu_rank eq '보통'}">
							               <td><img src="${pageContext.request.contextPath }/assets/images/priority/Urgency_usually.png" style="height: 20px"></td>
							              </c:if>
							              <c:if test="${issueboardList.isu_rank eq '긴급'}">
							               <td><img src="${pageContext.request.contextPath }/assets/images/priority/Urgency_Emergency.png" style="height: 20px"></td>
							              </c:if>
						               <td>${issueboardList.isu_title }</td>
						               <td>${issueboardList.isu_writer }</td>
						               <td>${issueboardList.isu_manager }</td>
						               <td>${fn:substring(issueboardList.isu_reg_date,0, 10) }</td>
						               <td>${issueboardList.isu_end_ok }</td>
						            </tr>
						         </c:forEach>
					         </c:if>
					         <c:if test="${empty issueboardList }">
					         		<tr>
						               <td></td>
						               <td><font style="color: red">등록된 이슈글이 없습니다.</font></td>
						            </tr>
					         </c:if>
                         </tbody>
                     </table>
...
<div style="margin-left: auto;">
	<form action="${pageContext.request.contextPath }/user/issuetracker/issueboard/issueboardList.do?pro_code=${pro_code}&pro_name=${param.pro_name }" method="post" class="form-inline pull-right">
		...
		&nbsp;&nbsp;<button  type="button" class="btn btn-inverse btn-sm" id="issueboardRegiBtn"><i class="ti-pencil-alt"></i>&nbsp;글쓰기</button>
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	</form>
</div>
<br/>
