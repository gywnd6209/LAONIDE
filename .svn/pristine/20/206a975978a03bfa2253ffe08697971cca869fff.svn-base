...
<!--    <form name="searchFrm" action="" method="post" class="form-inline pull-right"> -->
   <form action="${pageContext.request.contextPath }/admin/qaboard/qaboardList.do" method="post" class="form-inline pull-right">
...
$(function(){
	...
	$('#qaboardTBL tr').click(function(){
		...
		if(eval('${!empty LOGIN_ADMININFO}')){ 
			...
			$(location).attr('href', '${pageContext.request.contextPath}/admin/qaboard/qaboardView.do?bo_no='+bo_no+'&search_keyword=' + search_keyword 
	...
	$('#btn1').click(function(){
		 if(eval('${!empty LOGIN_ADMININFO}')){  
			$(location).attr('href', '${pageContext.request.contextPath}/admin/qaboard/qaboardForm.do');
		 }else{
	    	   alert("로그인해주세요.");
		         BootstrapDialog.show({
		             title: '경고',
