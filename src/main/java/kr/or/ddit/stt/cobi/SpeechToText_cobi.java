 ...
 *
 */
public class SpeechToText_cobi {
	private static final String API_KEY = "EL67KvwVFZTFw6rK-D-5euUw9kUCd4_m9A_jHEDwYUGB";
	...
	private List<String> temp_arr = new ArrayList<String>();
...
//		ServiceCall<SpeechModel> serviceCall_kor = service.getLanguageModel("ko-KR_BroadbandModel");
	...
	private void setOption() {
		...
		try {
			FileInputStream fis = 
					new FileInputStream(
							new File(getClass().getResource("hello.mp3").getPath()));
...
//					if(i==speechResults.getResults().size()-1){
//						test = text;
//						temp_arr.add("마지막"+text);
//					}
//				}
//				System.out.println("마지막"+test);
			}
			
			// 연결 종료시 처리할 내용
			@Override
			public void onDisconnected() {
				String last = temp_arr.get(temp_arr.size()-1);
				System.out.println(last);
				
				if(last.matches(".*[안녕|아나운서|입니다].*")){
	...
	public static void main(String[] args) {
		SpeechToText_cobi speechToText = new SpeechToText_cobi();
		speechToText.setService();
		speechToText.setHeader();
//		speechToText.getModel(); //-> 한국어 기능인데 다시 알아봐야함;
