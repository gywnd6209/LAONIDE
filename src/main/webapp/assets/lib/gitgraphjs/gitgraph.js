(function() {
    ...
    GitGraph.prototype.render = function() {
        ...
        // Add some margin
        this.context.translate(this.marginX, this.marginY - 20); // goorm: pull up
    ...
    function Branch(options) {
        ...
        this.parentCommit = this.parentBranch && this.parentBranch.commits.slice(-1)[0]; // goorm: parent branch's last commit
        ...
        for (var i = 0, point; !!(point = this.path[i]); i++) {
            if (point.type === "start") {
                ...
                if (this.template.branch.mergeStyle === "bezier") {
                    ...
                    // goorm start: connect line more like trapezoid instead of connecting dots directly
                    var branch_start_point = point.y;

                    if (path.x !== point.x && path.y !== point.y) {
                        branch_start_point = path.y - this.template.commit.spacingY;
                    }

                    this.context.bezierCurveTo(
                        path.x - this.template.commit.spacingX / 2, path.y - this.template.commit.spacingY / 2,
                        point.x + this.template.commit.spacingX / 2, branch_start_point + this.template.commit.spacingY / 2,
                        point.x, branch_start_point
                    );

                    if (branch_start_point !== point.y) {
    ...
    Branch.prototype.commit = function(options) {
        ...
        options.parent = this.parent;
        options.parentCommit = options.parentCommit || this.parentCommit || this.commits.slice(-1)[0]; // goorm: get parent commit at branch started
        ...
        if (isFirstBranch && isPathBeginning) {
            var parent = {
                x: commit.parentCommit.x, // goorm: start branch from parent commit
                y: commit.parentCommit.y,
            ...
            parent.type = "join";
            this.parentBranch && this.parentBranch.path.push(parent); // goorm: prevent error
     ...
     * @this Branch
     **/
    Branch.prototype._delete = function() { // goorm: don't use keyword -> error occurs in yui-compressor
    ...
    Commit.prototype.render = function() {
        ...
        // this.context.font = this.messageFont;	// hidden by goorm: moved in tag below

        // Tag
        var tagWidth = this.template.commit.tag.spacingX;
        if (this.tag !== null) {
            this.parent.tagNum++;
            this.context.font = this.tagFont; // goorm: apply tag's font before drawing tag
            var textWidth = this.context.measureText(this.tag).width + 18; // goorm: add some padding
            if (this.template.branch.labelRotation !== 0) {
                ...
                drawTextBG(this.context,
                    ((this.parent.columnMax + 2) * this.template.commit.tag.spacingX), // goorm
                    // ((this.parent.columnMax + 1) * this.template.commit.tag.spacingX) - this.template.commit.tag.spacingX / 2 + textWidth / 2,
                    this.y - getFontHeight(this.tagFont) + 3, // goorm
                    // this.y - this.dotSize / 2,
                    this.tag, "black", this.tagColor, this.tagFont, 0);
            }
            tagWidth = (tagWidth < textWidth) ? textWidth + tagWidth : tagWidth; // goorm
            // tagWidth = (tagWidth < textWidth) ? textWidth : tagWidth;
        }

        this.context.font = this.messageFont; // goorm: apply message font after drawing tag
    ...
    function drawTextBG(context, x, y, text, fgcolor, bgcolor, font, angle) {
        ...
        context.font = font;
        var width = context.measureText(text).width + 8; // goorm
        // var width = context.measureText( text ).width;
        var height = getFontHeight(font);

        context.beginPath();
        context.rect(0, 0, width, height + 2); // goorm
        // context.rect( -(width / 2) - 4, -(height / 2) + 2, width + 8, height + 2 );
        context.fillStyle = bgcolor;
        context.fill();
        // context.lineWidth = 2;	// hidden by goorm
        // context.strokeStyle = "black";
        // context.stroke();

        context.fillStyle = fgcolor;
        context.fillText(text, width / 2, height - 3); // goorm
