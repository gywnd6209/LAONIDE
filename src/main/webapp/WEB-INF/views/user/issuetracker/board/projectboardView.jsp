...
var bo_mem_id 	= '${proboardInfo.mem_id}';
var bo_writer 	= '${proboardInfo.bo_writer}';

$(function(){
	
    $('#deleteBtn').click(function() {
		if(mem_id == bo_mem_id){
			$(location).attr('href', '${pageContext.request.contextPath }/user/issuetracker/board/deleteProboard.do?bo_no=${proboardInfo.bo_no}&pro_code=${pro_code}&pro_name=${param.pro_name }');
		}else{
    ...
    $('form[name=updateFrm]').submit(function() {
		if(mem_id == bo_mem_id){
			...
			$(this).attr('action', '${pageContext.request.contextPath }/user/issuetracker/board/updateProboard.do?pro_code=${pro_code}&pro_name=${param.pro_name }');
			$(this).append('<input type="hidden" name="bo_no" value="${proboardInfo.bo_no}"></input>');
    ...
    /*
    $('#proReplyRegiBtn').click(function() {
		...
		// 댓글 등록 ajax
    	 $.ajax({
	         type : 'POST',
	         url : '${pageContext.request.contextPath}/user/issuetracker/board/insertProboardReply.do',
	         dataType : 'json', 
	         data : { 
	        	 bo_cont : 		$('#replyText').val(),
	        	 bo_writer : 	'${LOGIN_MEMBERINFO.mem_nick}',
	        	 mem_id : 		'${LOGIN_MEMBERINFO.mem_id}',
	        	 bo_group : 	${proboardInfo.bo_no},
	        	 bo_no : 		${proboardInfo.bo_no},
	        	 pro_code : 	'${pro_code}',
	        	 },
...
function fn_updateReply(bo_no){
	...
	// 댓글 수정 ajax
	 $.ajax({
        type : 'POST',
        url : '${pageContext.request.contextPath}/user/issuetracker/board/updateProboardReply.do',
        dataType : 'json', 
        data : { 
       	 bo_cont : 		editContent,
       	 bo_writer : 	'${LOGIN_MEMBERINFO.mem_nick}',
...
function deleteReply(bo_no) {
	...
	$.ajax({
		type : 'POST',
		url : '${pageContext.request.contextPath}/user/issuetracker/board/deleteProboardReply.do',
		dataType : 'json', 
		data : { 
			bo_no : 		bo_no
		},
           ...
           <h4 class="sub-title"><i class="ti-notepad"></i> 게시글보기</h4>
         	 <form name="updateFrm" class="form-horizontal" role="form" action="" method="post" enctype="multipart/form-data">
               <div class="form-group row">
                   ...
                   <div class="col-sm-10">
                       <input type="text" onKeyUp="javascript:fnChkByte_title(this,'200')" class="form-control"  id="bo_title" name="bo_title" value="${proboardInfo.bo_title}"></input>
                   </div>
               </div>
               <div class="form-group row">
                   <label class="col-sm-2 col-form-label" style="text-align: center">글쓴이</label>
                   <div class="col-sm-3">
                   <label class="col-form-label" style="text-align: left;" id="bo_date" name="bo_date">${proboardInfo.bo_writer}</label>
<!--                        <input type="text" disabled="disabled" class="form-control"  id="bo_writer" name="bo_writer" /> -->
               ...
               <div class="form-group row">
                   ...
                   <div class="col-sm-3">
                   	<label class="col-form-label" style="text-align: left;" id="bo_date" name="bo_date">${fn:substring(proboardInfo.bo_date,0, 10) }</label>
...
// 	console.log('fnChkByte_View : ' +  byteRtn);
	document.getElementById('byteInfo').innerText = byteRtn;
                       ...
                       <textarea id="bo_cont" name="bo_cont" style="height: 200px" rows="5" cols="5" class="form-control"
                        onKeyUp="javascript:fnChkByte(this,'1000')">${proboardInfo.bo_cont}</textarea>
                       <span id="byteInfo">0 &nbsp;</span>&nbsp;/&nbsp;1000bytes 
                   </div>
               </div>
               
<!-- 첨부파일 시작-->
<c:if test="${!empty proboardInfo.items }">
		<div class="form-group">
			<label class="col-sm-2 col-form-label" style="text-align: center">첨부파일</label>
			<c:forEach items="${proboardInfo.items }" var="fileItem"   varStatus="stat">
				<span style="text-align: left; padding-right: 20px;"> 
					<i class="ti-save">&nbsp;${fileItem.file_name}
						<a href="${pageContext.request.contextPath}/user/issuetracker/board/proDownload.do?file_seq=${fileItem.file_seq }">
...
<!-- 				</div> -->
<!-- 				<div class="form-group row"> -->
<!-- 					<label class="col-sm-2 col-form-label"  style="text-align: center" for="file02">첨부파일2</label> -->
<!-- 					<div class="col-sm-10"> -->
<!-- 						<input type="file" class="filestyle" id="files" name="files" data-buttonName="btn-primary"> -->
<!-- 					</div> -->
<!-- 				</div> -->
...
<!-- 			<button type="button" class="btn waves-effect waves-light btn-info" id="goListBtn"><i class="icofont icofont-info-square"></i>목록</button> -->
			...
			<c:if test="${LOGIN_MEMBERINFO.mem_id eq proboardInfo.mem_id}">
 	...
 	<div id="replyList" >
 		...
 		<h4 class="sub-title"><i class="ti-comments"></i> 댓글<label class="badge badge-info">${fn:length(proboardReplyList)} </label></h4>
	 		<div  class="row" style="text-align: center; margin: auto;">
	 			<c:if test="${empty proboardReplyList}">
	 			...
	 			<c:forEach items="${proboardReplyList}" var="proReplyInfo" varStatus="status">
	 			...
	 			<div class="col-sm-12" style="padding-left: 90px; padding-right:90px; padding-top: 2px;  background-color: #F0F0F0;">
	 				<div class="row" >
	 					...
	 					<div class="label-main" style=" float: left;">
	 						<c:if test="${LOGIN_MEMBERINFO.mem_id eq proReplyInfo.mem_id}">
                            <label class="label " style="background-color: #A27AAF;">  <font size="2pt" >${proReplyInfo.bo_writer}</font></label>
                            </c:if>
                            <c:if test="${LOGIN_MEMBERINFO.mem_id ne proReplyInfo.mem_id}">
                            <label class="label label-inverse">  <font size="2pt" >${proReplyInfo.bo_writer}</font></label>
                            </c:if>
                        
                            <font size="2pt" color="gray">
                            <fmt:parseDate value="${proReplyInfo.bo_date}" var="noticePostDate" pattern="yyyy-MM-dd"/>
...
<%--                             <fmt:formatDate value="${proReplyInfo.bo_date}"  type="date" dateStyle="full"/> --%>
							...
							&nbsp;
							<c:if test="${LOGIN_MEMBERINFO.mem_id eq proReplyInfo.mem_id}">
							<a href="javascript:void(0);" onclick="updateReply(${proReplyInfo.bo_no})"><i class="ti-marker-alt" style="color: #06908C;" ></i></a>
							<a href="javascript:void(0);" onclick='deleteReply(${proReplyInfo.bo_no})'><i class="ti-close" style="color: #F03E68;"></i></a>
                        	
							</c:if>
                         </div>
	 				</div>
	 				<div class="row"  id="div${proReplyInfo.bo_no}">
	 					<div style="padding-left: 15px; padding-top: 5px; width:100%; text-align:left;" id="${proReplyInfo.bo_no}"><input type="hidden" class="pCont" value="${proReplyInfo.bo_cont}">${proReplyInfo.bo_cont}</div>
	 				</div>
	 				<hr/>
	 				</tr>
	 			</div>
...
<!-- 				<form name="replyRegi" action="" method="post" enctype="multipart/form-data"> -->
				<div class="row col-sm-12" style="padding-left: 90px; padding-right:90px; padding-bottom: 10px;">
					...
					<div class="row">
					&nbsp;&nbsp;<button id="proReplyRegiBtn" onclick="replyInsert();" type="button" class="btn btn-inverse btn-sm" ><i class="ti-pencil-alt"></i>&nbsp;댓글쓰기</button>
					</div>
				</div>
<!-- 				</form> -->
